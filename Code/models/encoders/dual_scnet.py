...
class MemoryModulev3(nn.Module):
    def __init__(self, dim, num_head=8, window=7, norm_cfg=dict(type='SyncBN', requires_grad=True)):
        ...
        self.a = nn.Linear(dim, dim)
        self.l = nn.Linear(dim, dim)
        self.conv = nn.Conv2d(dim, dim, 7, padding=3, groups=dim) #3,5,9
    ...
    def forward(self, x):
        ...
        if self.window != 0:
            ...
            b = self.conv_att(x.permute(0, 3, 1, 2))
            b = b.permute(0, 2, 3, 1)
            kv = self.kv(b)
            kv = kv.reshape(B, H*W, 2, self.num_head, C // self.num_head).permute(2, 0, 3, 1, 4)
            k, v = kv.unbind(0)
...
def scnet_m(pretrained=False, **kwargs):   # 81.5
    model = SCNet(dims=[64, 144, 288, 512], mlp_ratios=[8, 8, 4, 4], depths=[3, 3, 12, 3], num_heads=[1, 2, 4, 8], windows=[0, 0, 0, 0], **kwargs)
    # model.default_cfg = _cfg()
    if pretrained:
        model = load_model_weights(model, 'scnet', kwargs)
    return model

def scnet_s(pretrained=False, **kwargs):   # 81.5
