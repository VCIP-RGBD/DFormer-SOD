...
class MemoryModulev3(nn.Module):
    def __init__(self, dim, num_head=8, window=7, norm_cfg=dict(type='BN', requires_grad=True),drop_depth=False):
        super().__init__()
...
class Block(nn.Module):
    def __init__(self, index, dim, num_head, norm_cfg=dict(type='BN', requires_grad=True), mlp_ratio=4.,block_index=0, last_block_index=50, window=7, dropout_layer=None,drop_depth=False):
        ...
        if block_index>last_block_index:
            window=0 
        self.attn = MemoryModulev3(dim, num_head, window=window, norm_cfg=norm_cfg,drop_depth=drop_depth)
...
class SCNet(BaseModule):
    ...
    def init_weights(self,pretrained):
        ...
        _state_dict=torch.load(pretrained)['state_dict_ema']
...
def rgbd_n(pretrained=False, **kwargs):   # 81.5
...
def rgbd_small(pretrained=False, **kwargs):   # 81.0
...
def rgbd_base(pretrained=False,drop_path_rate=0.1, **kwargs):   # 82.1
    model = SCNet(dims=[64, 128, 256, 512], mlp_ratios=[8, 8, 4, 4], depths=[3, 3, 12, 2], num_heads=[1, 2, 4, 8], windows=[0, 7, 7, 7],drop_path_rate=drop_path_rate, **kwargs)
   
    if pretrained:
        model = load_model_weights(model, 'scnet', kwargs)
    return model

def rgbd_base_kuan(pretrained=False,drop_path_rate=0.1, **kwargs):   # 82.1
    model = SCNet(dims=[96, 192, 288, 576], mlp_ratios=[8, 8, 4, 4], depths=[3, 3, 12, 2], num_heads=[1, 2, 4, 8], windows=[0, 7, 7, 7],drop_path_rate=drop_path_rate, **kwargs)
    if pretrained:
        model = load_model_weights(model, 'scnet', kwargs)
    return model

def rgbd_large(pretrained=False,drop_path_rate=0.1, **kwargs):   # 82.1
    model = SCNet(dims=[64, 128, 256, 512], mlp_ratios=[8, 8, 4, 4], depths=[3, 5, 27, 3], num_heads=[1, 2, 4, 8], windows=[0, 7, 7, 7],drop_path_rate=drop_path_rate, **kwargs)
    if pretrained:
        model = load_model_weights(model, 'scnet', kwargs)
    return model