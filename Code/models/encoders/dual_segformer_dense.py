...
class RGBXTransformer(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dims=[64, 128, 256, 512], 
        ...
        cur = 0
        # 
        self.FRMs1 = nn.ModuleList([FRM(dim=embed_dims[0], reduction=1)
        ...
        self.block1 = nn.ModuleList([Block(
            ...
            for i in range(depths[0])])
        self.norm1 = nn.ModuleList([norm_layer(embed_dims[0]) for i in range(depths[0])])
        ...
        self.extra_block1 = nn.ModuleList([Block(
            ...
            sr_ratio=sr_ratios[0])
            for i in range(depths[0])])
        self.extra_norm1 = nn.ModuleList([norm_layer(embed_dims[0]) for i in range(depths[0])])
        cur += depths[0]

        #
        self.FRMs2 = nn.ModuleList([FRM(dim=embed_dims[1], reduction=1)
        ...
        self.block2 = nn.ModuleList([Block(
            ...
            for i in range(depths[1])])
        self.norm2 = nn.ModuleList([norm_layer(embed_dims[1]) for i in range(depths[1])])
        ...
        self.extra_block2 = nn.ModuleList([Block(
            ...
            sr_ratio=sr_ratios[1])
            for i in range(depths[1])])
        self.extra_norm2 = nn.ModuleList([norm_layer(embed_dims[1]) for i in range(depths[1])])

        cur += depths[1]

        self.FRMs3 = nn.ModuleList([FRM(dim=embed_dims[2], reduction=1)
        ...
        self.block3 = nn.ModuleList([Block(
            ...
            for i in range(depths[2])])
        self.norm3 = nn.ModuleList([norm_layer(embed_dims[2]) for i in range(depths[2])])
        ...
        self.extra_block3 = nn.ModuleList([Block(
            ...
            sr_ratio=sr_ratios[2])
            for i in range(depths[2])])
        self.extra_norm3 = nn.ModuleList([norm_layer(embed_dims[2]) for i in range(depths[2])])

        cur += depths[2]

        self.FRMs4 = nn.ModuleList([FRM(dim=embed_dims[3], reduction=1)
        ...
        self.block4 = nn.ModuleList([Block(
            ...
            for i in range(depths[3])])
        self.norm4 = nn.ModuleList([norm_layer(embed_dims[3]) for i in range(depths[3])])
        ...
        self.extra_block4 = nn.ModuleList([Block(
            ...
            sr_ratio=sr_ratios[3])
            for i in range(depths[3])])
        self.extra_norm4 = nn.ModuleList([norm_layer(embed_dims[3]) for i in range(depths[3])])
    ...
    def forward_features(self, x_rgb, x_e):
        ...
        for i in range(len(self.block1)):
            # print(x_rgb.shape)
            x_rgb = self.block1[i](x_rgb, H, W)
            x_e = self.extra_block1[i](x_e, H, W)
            x_rgb = self.norm1[i](x_rgb)
            x_e = self.extra_norm1[i](x_e)
            x_rgb = x_rgb.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()
            x_e = x_e.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()
            x_rgb, x_e = self.FRMs1[i](x_rgb, x_e)
            if i == len(self.block1)-1:
                x_fused = self.FFMs[0](x_rgb, x_e)
                outs.append(x_fused)
            else:
                C=x_rgb.shape[1]
                x_rgb = x_rgb.reshape(B,C,-1).permute(0, 2,1).contiguous()
                x_e = x_e.reshape(B,C,-1).permute(0, 2,1).contiguous()
        ...
        for i in range(len(self.block2)):
            x_rgb = self.block2[i](x_rgb, H, W)
            x_e = self.extra_block2[i](x_e, H, W)
            x_rgb = self.norm2[i](x_rgb)
            x_e = self.extra_norm2[i](x_e)
            x_rgb = x_rgb.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()
            x_e = x_e.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()
            x_rgb, x_e = self.FRMs2[i](x_rgb, x_e)
        ...
        for i in range(len(self.block3)):
            x_rgb = self.block3[i](x_rgb, H, W)
            x_e = self.extra_block3[i](x_e, H, W)
            x_rgb = self.norm3[i](x_rgb)
            x_e = self.extra_norm3[i](x_e)
            x_rgb = x_rgb.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()
            x_e = x_e.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()
            x_rgb, x_e = self.FRMs3[i](x_rgb, x_e)
        ...
        for i in range(len(self.block4)):
            x_rgb = self.block4[i](x_rgb, H, W)
            x_e = self.extra_block4[i](x_e, H, W)
            x_rgb = self.norm4[i](x_rgb)
            x_e = self.extra_norm4[i](x_e)
            x_rgb = x_rgb.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()
            x_e = x_e.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()
            x_rgb, x_e = self.FRMs4[i](x_rgb, x_e)
