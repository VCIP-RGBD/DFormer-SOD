...
class SCNet(BaseModule):
    def __init__(self, in_channels=3, depths=(2, 2, 8, 2), dims=(32, 64, 128, 256), out_indices=(0, 1, 2, 3), windows=[7, 7, 7, 7], norm_cfg=dict(type='SyncBN', requires_grad=True),
        ...
        self.e_frms = nn.ModuleList()
        self.e_norm_stages = nn.ModuleList()
        ...
        for i in range(len(dims)):
            ...
            norm_stage = nn.Sequential(
                *[LayerNorm(dims[i], eps=1e-6, data_format="channels_first") for j in range(depths[i])]
            )
            e_norm_stages = nn.Sequential(
                *[LayerNorm(dims[i], eps=1e-6, data_format="channels_first") for j in range(depths[i])]
            )
            e_frm = nn.Sequential(
                *[FRM(dim=dims[i], reduction=1) for j in range(depths[i])]
            )
            self.stages.append(stage)
            self.e_stages.append(e_stage)
            self.e_frms.append(e_frm)
            self.e_norm_stages.append(e_norm_stages)
            self.norm_stages.append(norm_stage)
            cur += depths[i]
        ...
        self.FRMs = nn.ModuleList([
                    FRM(dim=dims[0], reduction=1),
                    FRM(dim=dims[1], reduction=1),
                    FRM(dim=dims[2], reduction=1),
                    FRM(dim=dims[3], reduction=1)])
    ...
    def forward(self, x,x_e):
        ...
        for i in range(4):
            ...
            for j in range(len(self.stages[i])):#,self.e_stages[i],self.e_frms[i]:
                x = x.permute(0, 2, 3, 1)
                ...
                # print('+++++++++++++++')
                # print(self.e_frms[i][j])
                if j == len(self.stages[i])-1:
                    pass
                else:
                    x=self.norm_stages[i][j](x)
                    x_e=self.e_norm_stages[i][j](x_e)
                    x,x_e = self.e_frms[i][j](x,x_e)
