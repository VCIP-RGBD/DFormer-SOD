...
class RGBXTransformer(nn.Module):
    ...
    def forward_features(self, x_rgb, x_e):
        ...
        x_rgb_t, x_e_t = self.FRMs[0](x_rgb, x_e)
        x_fused = self.FFMs[0](x_rgb_t, x_e_t)
        outs.append(x_fused)
        ...
        x_rgb = x_rgb.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()
        x_e = x_e.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()
        x_rgb_t, x_e_t = self.FRMs[1](x_rgb, x_e)
        x_fused = self.FFMs[1](x_rgb_t, x_e_t)
        outs.append(x_fused)
        ...
        x_rgb = x_rgb.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()
        x_e = x_e.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()
        x_rgb_t, x_e_t = self.FRMs[2](x_rgb, x_e)
        x_fused = self.FFMs[2](x_rgb_t, x_e_t)
        outs.append(x_fused)
