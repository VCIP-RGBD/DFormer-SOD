...
class MemoryModulev3(nn.Module):
    def __init__(self, dim, num_head=8, window=7, norm_cfg=dict(type='BN', requires_grad=True)):
        super().__init__()
...
class Block(nn.Module):
    def __init__(self, index, dim, num_head, norm_cfg=dict(type='BN', requires_grad=True), mlp_ratio=4.,block_index=0, last_block_index=50, window=7, dropout_layer=None):
        super().__init__()
        ...
        if block_index>last_block_index:
            window=0 
        self.attn = MemoryModulev3(dim, num_head, window=window, norm_cfg=norm_cfg)
...
def rgbd_s(pretrained=False, **kwargs):   # 81.5
...
def rgbd_base(pretrained=False,drop_path_rate=0.1, **kwargs):   # 81.5
    model = SCNet(dims=[64, 144, 320, 512], mlp_ratios=[8, 8, 4, 4], depths=[3, 5, 12, 2], num_heads=[1, 2, 4, 8], windows=[0, 7, 7, 7],drop_path_rate=drop_path_rate, **kwargs)
