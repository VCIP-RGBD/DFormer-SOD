...
logging.info("DFormer-RGBD-SOD-Train")
logging.info("Config")
...
def train(train_loader, model, optimizer, epoch,save_path):
    ...
    try:
        for i, (images, gts, depths) in enumerate(train_loader, start=1):
            ...
            loss1    = structure_loss(pre_res, gts) 
            ...
            if i % 50 == 0 or i == total_step or i==1:
                print('{} Epoch [{:03d}/{:03d}], Step [{:04d}/{:04d}], Loss1: {:.4f} Loss2: {:0.4f} Loss3: {:0.4f}'.
                    format(datetime.now(), epoch, opt.epoch, i, total_step, loss1.data, loss1.data,  loss1.data))
                logging.info('#TRAIN#:Epoch [{:03d}/{:03d}], Step [{:04d}/{:04d}], Loss1: {:.4f} Loss2: {:0.4f} Loss3: {:0.4f}'.
                    format( epoch, opt.epoch, i, total_step, loss1.data, loss1.data, loss1.data))
